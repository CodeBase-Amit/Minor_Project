...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const handleActiveEditorChanged = useCallback(
        ({ socketId, fileId, cursor }: { socketId: string, fileId: string, cursor?: number }) => {
            // Update the user's active file in the users state
            setUsers((prevUsers) =>
                prevUsers.map((user) => {
    ...
    useEffect(() => {
        ...
        socket.on(SocketEvent.ACTIVE_EDITOR_CHANGED, handleActiveEditorChanged)
        ...
        return () => {
            ...
            socket.off(SocketEvent.ACTIVE_EDITOR_CHANGED)
        }
    }, [
        handleDirCreated,
        handleDirDeleted,
