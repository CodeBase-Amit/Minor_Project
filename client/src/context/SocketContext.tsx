...
const SocketProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        ...
        socket.on(SocketEvent.USER_JOINED, ({ user }) => {
            console.log('User joined:', user);
            setUsers(prevUsers => {
                if (prevUsers.find(u => u.socketId === user.socketId)) {
                    return prevUsers;
                }
        ...
        socket.on(SocketEvent.USERS_IN_ROOM, ({ users: roomUsers }) => {
            console.log('Received users in room:', roomUsers);
        ...
        return () => {
            ...
            socket.off(SocketEvent.USERS_IN_ROOM)
        }
    }, [
        handleDrawingSync,
        handleError,
