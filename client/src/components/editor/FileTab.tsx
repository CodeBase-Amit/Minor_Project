...
function FileTab() {
    ...
    const changeActiveFile = (fileId: string) => {
        ...
        if (file) {
            ...
            // Emit active editor changed event
            if (socket && currentUser.roomId) {
                socket.emit(SocketEvent.ACTIVE_EDITOR_CHANGED, {
    ...
    // When the active file changes, emit the active editor changed event
    useEffect(() => {
        if (socket && currentUser.roomId && activeFile) {
            socket.emit(SocketEvent.ACTIVE_EDITOR_CHANGED, {
                roomId: currentUser.roomId,
                fileId: activeFile.id,
            })
        }
    }, [activeFile, currentUser.roomId, socket])
